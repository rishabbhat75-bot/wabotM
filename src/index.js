...
Avoid walls of text. Provide absolute maximum intelligence, logic, and structure. DO NOT introduce yourself or state your model name. Just answer.`;
        ...
        if (isImageIntent) {
            ...
            try {
                 const imgBuffer = await generateImageGemini(fullPrompt);
                 await sock.sendMessage(chatJid, { image: imgBuffer, caption: "🎨 *Here is your generated image!*" }, { quoted: msg });
                 await sock.sendMessage(chatJid, { delete: sentMsg.key });
            } catch (err) {
                 await sock.sendMessage(chatJid, { text: `❌ _Failed to generate image: ${err.message}_` }, { edit: sentMsg.key });
            }
            return;
        }
